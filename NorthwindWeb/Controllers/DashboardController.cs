    ...
    public class DashboardController : Controller
    {
        ...
        private List<LastTenOrders> LastTenOrder()
        {
            ...
            foreach (var itemOrder in order)
            {
                ...
                if (pastTime.TotalMinutes <= 60) { lastTenOrdersElement.Ago = "Last " + Convert.ToString(decimal.Round(Convert.ToDecimal(pastTime.TotalMinutes), 0)) + " minutes"; }
                else if (pastTime.TotalHours <= 24) { lastTenOrdersElement.Ago = "Last " + Convert.ToString(decimal.Round(Convert.ToDecimal(pastTime.TotalHours), 0)) + " hours"; }
                else if (pastTime.TotalDays <= 30) { lastTenOrdersElement.Ago = "Last " + Convert.ToString(decimal.Round(Convert.ToDecimal(pastTime.TotalDays), 0)) + " days"; }
                else { lastTenOrdersElement.Ago = "More than a month ago"; }
