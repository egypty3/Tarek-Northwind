    ...
    public class ReportsController : Controller
    {
        ...
        public ActionResult Index([Bind(Include = "Username,Password")] ReportLoginViewModel login)
        {
            try
            {
                ...
                doc = web.Load($"{reportServer}?%2f{reportServerDir}", "GET", new WebProxy() { UseDefaultCredentials = true },
                    new NetworkCredential(login.Username, login.Password));

                var links = doc.DocumentNode.SelectNodes("//a");
                if (links == null)
                {
                    ViewBag.ErrorMessage = "Your name or password was not entered correctly.";
                    return View("LogIn");
                }
            ...
            catch (WebException e)
            {
                ...
                throw new WebException("I did not receive any response from the server. Verify that the server address is spelled correctly or that it is started.");
            }
            ...
            {
                logger.Error(e.ToString());
                throw new Exception("The server address is not in a valid format");
            }
